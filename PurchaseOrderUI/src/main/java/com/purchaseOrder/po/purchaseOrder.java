package com.purchaseOrder.po;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.ModelMap;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.servlet.ModelAndView;

import com.purchaseOrder.dao.ProductsDao;
import com.purchaseOrder.model.Products;

@Controller
public class purchaseOrder {

	@Autowired
	ProductsDao productdao;
	
	@RequestMapping(value="/purchaseOrder",method=RequestMethod.GET)
	public String getproducts(ModelMap map)
	{
		map.addAttribute("purchaseObj", new Products());
		return "raisePo";
		
	}
	
	@RequestMapping(value="/items",method=RequestMethod.GET)
    public ModelAndView raisePurchaseOrder(){
           List<Products> product=productdao.viewAllProducts();
           ModelAndView mv=new ModelAndView("SellerSuccess");
           mv.addObject("purchaseObj", product);
      return mv;
    
	}
	

}
